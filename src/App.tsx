...
const App: React.FC = () => {
  ...
  const [cart, setCart] = useState<{ id: string; color: string; price: number }[]>([]);
          ...
          {isCartOpen && (
            <>
              ...
              <motion.div 
                ...
                className="absolute top-0 right-0 h-full w-full md:w-[450px] bg-brand-black border-l border-white/10 z-[110] p-12 flex flex-col"
              >
                <div className="flex justify-between items-center mb-12">
                  <h2 className="text-4xl font-black uppercase tracking-tighter italic">Your Cart</h2>
                  <button 
                    onClick={() => setIsCartOpen(false)}
                    className="text-brand-gray hover:text-white transition-colors uppercase text-xs font-black tracking-widest"
                  >
                    Close
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar space-y-8">
                  {cart.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-40">
                      ...
                      <p className="uppercase tracking-widest text-xs font-black">Your cart is empty</p>
                    </div>
                  ) : (
                    cart.map((item) => (
                      <div key={item.id} className="flex gap-6 items-center group">
                        <div 
                          className="w-24 h-24 rounded-2xl flex items-center justify-center border border-white/10 relative overflow-hidden"
                          style={{ backgroundColor: `${item.color}11` }}
                        >
                          <div className="w-16 h-16 rounded-full" style={{ backgroundColor: item.color, boxShadow: `0 0 20px ${item.color}44` }}></div>
                        </div>
                        <div className="flex-1">
                          <h3 className="font-black uppercase tracking-tight text-xl">Spalding Elite</h3>
                          <p className="text-xs text-brand-gray uppercase tracking-widest mb-2">Color: {colors.find(c => c.value === item.color)?.name}</p>
                ...
                {cart.length > 0 && (
                  <div className="mt-12 pt-12 border-t border-white/10 space-y-8">
                    <div className="flex justify-between items-end">
                      <span className="text-brand-gray uppercase text-xs font-black tracking-widest">Total Amount</span>
                      <span className="text-4xl font-black tracking-tighter italic">
                        ${cart.reduce((sum, item) => sum + item.price, 0).toFixed(2)}
                      </span>
                    </div>
                    <button 
                      className="w-full h-16 rounded-xl font-black uppercase tracking-[0.2em] text-sm hover:scale-[1.02] active:scale-95 transition-all duration-300"
                      style={{ 
                        backgroundColor: themeColor,
